...
export const metadata = {
  title: 'QORE — Control de Asistencia Inteligente | Variante C',
};

export default function LandingV3() {
  return (
    <div className="min-h-screen bg-[#f0fdff] text-slate-900 overflow-hidden">
      {/* ─── Navigation — Solid white with border ────────── */}
      <nav className="fixed top-0 left-0 right-0 z-50 flex justify-center pt-4 px-4">
        <div className="flex h-14 w-full max-w-6xl items-center rounded-full bg-white/80 backdrop-blur-xl border border-slate-200/60 px-6" style={{ boxShadow: '0 4px 6px rgba(0,0,0,0.04), 0 12px 24px rgba(0,0,0,0.06), 0 24px 48px rgba(0,0,0,0.04)' }}>
          {/* Logo — left */}
          ...
          <div className="hidden md:flex items-center gap-2 mx-auto">
            {[
              ...
              <span key={item.label} className="flex items-center gap-2">
                <a href={item.href} className="text-sm text-[#000d2a] hover:text-[#2dd4ff] transition-colors">{item.label}</a>
                {i < arr.length - 1 && <span className="text-slate-300 text-[8px]">●</span>}
              </span>
            ))}
          </div>
      ...
      {/* ─── Hero — Offsitee style ──────────────────────── */}
      <section className="relative pt-16 pb-0 lg:pt-24 lg:pb-0">
        {/* Background — grid + cyan glow + white spotlight (Offsitee style) */}
        <div
          className="absolute inset-0 pointer-events-none"
          style={{
            backgroundImage: 'linear-gradient(to right, rgba(45,212,255,0.1) 1px, transparent 1px), linear-gradient(to bottom, rgba(45,212,255,0.1) 1px, transparent 1px)',
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col items-center text-center max-w-3xl mx-auto">
            {/* Social proof badge */}
            <div className="animate-fade-in-up flex items-center gap-3 rounded-full border border-slate-200 bg-white px-4 py-2 shadow-sm mb-8">
              <div className="flex -space-x-2">
                {['bg-[#2dd4ff]', 'bg-[#000d2a]', 'bg-emerald-500'].map((bg, i) => (
                  <div key={i} className={`h-7 w-7 rounded-full ${bg} border-2 border-white flex items-center justify-center text-[9px] font-bold text-white`}>
                    {['CM', 'LF', 'AV'][i]}
                  </div>
                ))}
              </div>
              <span className="text-sm text-slate-600">
                <span className="font-semibold text-slate-800">50+ empresas</span> confian en QORE
              </span>
            </div>

            {/* Title with italic accent */}
            <h1 className="animate-fade-in-up text-4xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight leading-[1.1] text-slate-900">
            ...
            {/* Subtitle */}
            <p className="animate-fade-in-up delay-200 mt-6 text-base sm:text-lg text-slate-500 leading-relaxed max-w-xl">
              Gestiona asistencia con QR y GPS en tiempo real. Elimina el fraude,
              automatiza reportes y simplifica la nomina — sin complicaciones.
            </p>

            {/* Double CTA */}
            <div className="animate-fade-in-up delay-300 mt-8 flex flex-wrap items-center justify-center gap-4">
              <a
                href="#waitlist"
          ...
          {/* ─── Dashboard Mockup ────────────────────────────── */}
          <div
            className="animate-blur-in delay-300 mt-16 mx-auto max-w-5xl relative rounded-3xl ring-4 ring-[#2dd4ff]/10"
            style={{ boxShadow: '0 4px 6px rgba(45,212,255,0.06), 0 10px 30px rgba(45,212,255,0.08), 0 25px 60px rgba(0,13,42,0.12), 0 40px 80px rgba(0,13,42,0.08)' }}
          >
            <div className="relative max-h-[680px] overflow-hidden rounded-3xl" style={{ maskImage: 'linear-gradient(to bottom, black 75%, transparent 100%)', WebkitMaskImage: 'linear-gradient(to bottom, black 75%, transparent 100%)' }}>
            <div className="flex">
              {/* Sidebar — real light theme */}
      ...
      <section id="features" className="py-24 lg:py-32 bg-white">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 max-w-5xl mx-auto">
            {[
              { icon: QrCode, title: 'Escaneo QR', desc: 'Codigos QR que se regeneran automaticamente. Registro de asistencia en segundos.', color: 'text-cyan-500 bg-cyan-50' },
              { icon: MapPin, title: 'Validacion GPS', desc: 'Presencia fisica verificada con radio configurable por sede.', color: 'text-emerald-500 bg-emerald-50' },
              { icon: BarChart3, title: 'Reportes', desc: 'Dashboard en tiempo real. Exporta en Excel, PDF y CSV.', color: 'text-violet-500 bg-violet-50' },
              { icon: Shield, title: 'Anti-Fraude', desc: 'Deteccion automatica de anomalias y sistema de alertas.', color: 'text-amber-500 bg-amber-50' },
              { icon: Globe, title: 'Multi-Sede', desc: 'Gestiona todas tus ubicaciones desde un solo panel central.', color: 'text-blue-500 bg-blue-50' },
              { icon: Lock, title: 'Seguridad', desc: 'Roles y permisos granulares, datos aislados por tenant.', color: 'text-rose-500 bg-rose-50' },
            ].map((f) => (
              <div key={f.title} className="flex flex-col items-start rounded-2xl border border-slate-200 bg-white p-6 hover:shadow-lg hover:border-[#2dd4ff]/20 transition-all">
